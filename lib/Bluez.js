	...
	async onInterfacesAdded(path, interfaces) {
		...
		if (matches.device) {
			if (interfaces['org.bluez.Adapter1']) {
				...
				this.emit('added-Adapter', { path : path, properties : interfaces['org.bluez.Adapter1'] });
			}

			if (interfaces['org.bluez.Device1']) {
				this.devices[matches.device[2]] = path;
				this.emit('added-Device', { path : path, properties : interfaces['org.bluez.Device1'] });
			}

			if (interfaces['org.bluez.Network1']) {
				this.emit('added-Network', { path : path, properties : interfaces['org.bluez.Network1'] });
			}

			if (interfaces['org.bluez.MediaControl1']) {
				let object = 'control' + matches.device[2];
		...
		if (matches.filesystem) {
			if (interfaces['org.bluez.Filesystem1']) {
				this.emit('added-Filesystem', { path : path, properties : interfaces['org.bluez.Filesystem1'] });
			}
		}

		if (matches.nowplaying_item) {
			if (interfaces['org.bluez.MediaItem1']) {
				let object = 'item' + matches.nowplayering_item[4];
		...
		if (matches.player) {
			if (interfaces['org.bluez.MediaPlayer1']) {
				let object = 'player' + matches.player[3];

				this.emit('added-MediaPlayer', { object : object, path : path, properties : interfaces['org.bluez.MediaPlayer1'] });

				if (typeof this.items[object] === 'undefined' || this.items[object] === null || this.items[object] === '') {
					this.players[object] = await this.getInterface('org.bluez', path, 'org.freedesktop.DBus.Properties');
					this.players[object].on('InterfacesAdded',   this.onInterfacesAdded.bind(this));
					this.players[object].on('PropertiesChanged', this.onPropertiesChanged.bind(this));
				}
