...
class Bluez extends EventEmitter {
	...
	async init() {
		...
		this.objectManager.on('PropertiesChanged', (prop1, prop2) => {
	...
	async onInterfacesAdded(path, interfaces) {
		...
		if (matches.device) {
			...
			if (interfaces['org.bluez.MediaControl1']) {
				...
				// this.controls['control' + matches.device[2]] = path;
				...
				this.controls['control' + matches.device[2]].on('PropertiesChanged', this.onInterfacesAdded.bind(this));
			}
		}
		...
		if (matches.fd) {
			if (interfaces['org.bluez.MediaTransport1']) {
				...
				// this.transports['fd' + matches.fd[3]] = path;
				...
				this.transports['fd' + matches.fd[3]].on('PropertiesChanged', this.onInterfacesAdded.bind(this));
			}
		}
		...
		if (matches.nowplaying_item) {
			if (interfaces['org.bluez.MediaItem1']) {
				...
				// this.items['item' + matches.nowplaying_item[3]] = path;
				...
				this.items['item' + matches.nowplaying_item[4]].on('PropertiesChanged', this.onInterfacesAdded.bind(this));
			}
		}
		...
		if (matches.player) {
			if (interfaces['org.bluez.MediaPlayer1']) {
				...
				// this.players['player' + matches.player[3]] = path;
				...
				this.players['player' + matches.player[3]].on('PropertiesChanged', this.onInterfacesAdded.bind(this));
			}
		}
	}
