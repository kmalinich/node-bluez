...
class Bluez extends EventEmitter {
	...
	async getMediaPlayer(address) {
		...
		if (this.media_players[address] && typeof this.media_players[address] !== 'string') {
			// Media player already created
			return this.media_players[address];
		}

		if (!this.media_players[address]) throw new Error('Media player not found');
		const interface_ = await this.getInterface('org.bluez', this.media_players[address], 'org.bluez.MediaPlayer1');
