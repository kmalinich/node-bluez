class Agent {
	constructor(bluez, DbusObject) {
		...
		this._DBusInterface = DbusObject.createInterface('org.bluez.Agent1');

		this._DBusInterface.addMethod('Release', {}, (callback) => {
			this.Release(callback);
		});

		this._DBusInterface.addMethod('RequestPinCode', {
			in : [ {
				type : 'o',
				name : 'device',
			} ],
			...
			this.RequestPinCode(device, callback);
		});

		this._DBusInterface.addMethod('DisplayPinCode', {
			in : [ {
				type : 'o',
				name : 'device',
			}, {
			...
			this.DisplayPinCode(device, pincode, callback);
		});

		this._DBusInterface.addMethod('RequestPasskey', {
			in : [ {
				type : 'o',
				name : 'device',
			} ],
			...
			this.RequestPasskey(device, code, callback);
		});

		this._DBusInterface.addMethod('DisplayPasskey', {
			in : [ {
				type : 'o',
				name : 'device',
			}, {
			...
			this.DisplayPasskey(device, passkey, entered, callback);
		});

		this._DBusInterface.addMethod('RequestConfirmation', {
			in : [ {
				type : 'o',
				name : 'device',
			}, {
			...
			this.RequestConfirmation(device, passkey, callback);
		});

		this._DBusInterface.addMethod('RequestAuthorization', {
			in : [ {
				type : 'o',
				name : 'device',
			} ],
		}, (device, callback) => {
			this.RequestAuthorization(device, callback);
		});

		this._DBusInterface.addMethod('AuthorizeService', {
			in : [ {
				type : 'o',
				name : 'device',
			}, {
			...
			this.AuthorizeService(device, uuid, callback);
		});

		this._DBusInterface.addMethod('Cancel', {}, (callback) => {
	...
	/*
		...
		There is no need to unregister the agent, because when this method gets called it has already been unregistered
	*/
	Release(callback) {
		callback();
	}
	...
	*/
	...
	/*
		...
		Possible errors:
			...
			org.bluez.Error.Rejected
	*/
	RequestPasskey(device, code, callback) {
		callback(null, 1234);
	}
